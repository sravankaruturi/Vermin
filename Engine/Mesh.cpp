...
namespace vermin {
    ...
    Mesh::Mesh(void *_dataPointer, size_t _dataStructureSize, unsigned _vertexCount, std::vector<unsigned> _indices) {
        ...
        if (usingIndexBuffer) {
            PE_GL(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO));
            PE_GL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * _indices.size(), &_indices[0],
                               GL_STATIC_DRAW));
        }
        ...
        for (int i = 0; i < sizeof(header_read) / sizeof(header_read[0]); i++) {
            ...
            switch (header_read[i]) {
                ...
                case 1:
                    PE_GL(glVertexAttribPointer(i, size_of_ints_per_input_vec, GL_FLOAT, GL_FALSE, _dataStructureSize,
                                                (void *) offset));
                ...
                case 2:
                    PE_GL(glVertexAttribIPointer(i, size_of_ints_per_input_vec, GL_INT, _dataStructureSize,
                                                 (void *) offset));
