...
namespace vermin {
    ...
    Entity::Entity(const std::string &_entityName, const std::string &_objectPath, const std::string &_shaderName,
                   glm::vec3 _boundingBoxLeast, glm::vec3 _boundingBoxHigh)
            : entityName(_entityName), shaderName(_shaderName), boundingBoxLeastVertex(_boundingBoxLeast),
              boundingBoxHighVertex(_boundingBoxHigh),
              boundingBox(BoundingBox(boundingBoxLeastVertex, boundingBoxHighVertex)) {
    ...
    bool Entity::CheckIfMouseOvered(const glm::vec3 _cameraPosition, const glm::vec3 _mouseRayDirection,
                                    float &_distance) const {
        // Checks if this entity is under the cursor. And updates the _distance to hold it.
        return this->boundingBox.CheckForCollisionWithRay(this->modelMatrix, this->scale, _cameraPosition,
