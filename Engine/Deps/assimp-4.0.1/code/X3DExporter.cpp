...
namespace Assimp {
    ...
    void
    ExportSceneX3D(const char *pFile, IOSystem *pIOSystem, const aiScene *pScene, const ExportProperties *pProperties) {
...
namespace Assimp {
    ...
    void X3DExporter::AttrHelper_Vec3DArrToString(const aiVector3D *pArray, const size_t pArray_Size,
                                                  string &pTargetString) {
        pTargetString.clear();
        pTargetString.reserve(pArray_Size * 6);// (Number + space) * 3.
        for (size_t idx = 0; idx < pArray_Size; idx++)
            pTargetString.append(
                    to_string(pArray[idx].x) + " " + to_string(pArray[idx].y) + " " + to_string(pArray[idx].z) + " ");
    ...
    void X3DExporter::AttrHelper_Vec2DArrToString(const aiVector2D *pArray, const size_t pArray_Size,
                                                  std::string &pTargetString) {
        pTargetString.clear();
    ...
    void X3DExporter::AttrHelper_Vec3DAsVec2fArrToString(const aiVector3D *pArray, const size_t pArray_Size,
                                                         string &pTargetString) {
        pTargetString.clear();
        pTargetString.reserve(pArray_Size * 4);// (Number + space) * 2.
        for (size_t idx = 0; idx < pArray_Size; idx++)
    ...
    void
    X3DExporter::AttrHelper_Col4DArrToString(const aiColor4D *pArray, const size_t pArray_Size, string &pTargetString) {
        pTargetString.clear();
        pTargetString.reserve(pArray_Size * 8);// (Number + space) * 4.
        for (size_t idx = 0; idx < pArray_Size; idx++)
            pTargetString.append(
                    to_string(pArray[idx].r) + " " + to_string(pArray[idx].g) + " " + to_string(pArray[idx].b) + " " +
    ...
    void X3DExporter::AttrHelper_Col3DArrToString(const aiColor3D *pArray, const size_t pArray_Size,
                                                  std::string &pTargetString) {
        pTargetString.clear();
        pTargetString.reserve(pArray_Size * 6);// (Number + space) * 3.
        for (size_t idx = 0; idx < pArray_Size; idx++)
    ...
    void X3DExporter::AttrHelper_Color3ToAttrList(std::list<SAttribute> &pList, const std::string &pName,
                                                  const aiColor3D &pValue, const aiColor3D &pDefaultValue) {
        string tstr;
    ...
    void X3DExporter::AttrHelper_FloatToAttrList(std::list<SAttribute> &pList, const string &pName, const float pValue,
                                                 const float pDefaultValue) {
        string tstr;
    ...
    void X3DExporter::NodeHelper_OpenNode(const string &pNodeName, const size_t pTabLevel, const bool pEmptyElement,
                                          const list<SAttribute> &pAttrList) {
        // Write indentation.
    ...
    void X3DExporter::Export_Node(const aiNode *pNode, const size_t pTabLevel) {
        ...
        if (!pNode->mTransformation.IsIdentity()) {
            ...
            auto Rotation2String = [this](const aiVector3D pAxis, const ai_real pAngle) -> string {
                string tstr = to_string(pAxis.x) + " " + to_string(pAxis.y) + " " + to_string(pAxis.z) + " " +
                              to_string(pAngle);
    ...
    void X3DExporter::Export_Material(const size_t pIdxMaterial, const size_t pTabLevel) {
        ...
        //
        {
            auto Color4ToAttrList = [&](const string &pAttrName, const aiColor4D &pAttrValue,
                                        const aiColor3D &pAttrDefaultValue) {
                string tstr;
            ...
            if (material.Get(AI_MATKEY_COLOR_AMBIENT, color3) == AI_SUCCESS)
                AttrHelper_FloatToAttrList(attr_list, "ambientIntensity", (color3.r + color3.g + color3.b) / 3.0f,
                                           0.2f);
            else if (material.Get(AI_MATKEY_COLOR_AMBIENT, color4) == AI_SUCCESS)
                AttrHelper_FloatToAttrList(attr_list, "ambientIntensity", (color4.r + color4.g + color4.b) / 3.0f,
                                           0.2f);
            ...
            if (material.Get(AI_MATKEY_SHININESS, tvalf) == AI_SUCCESS)
                AttrHelper_FloatToAttrList(attr_list, "shininess", tvalf, 0.2f);
        ...
        //
        {
            ...
            // repeatS="true" SFBool
            if (material.Get(AI_MATKEY_MAPPINGMODE_U_DIFFUSE(0), tvalb) == AI_SUCCESS)
                RepeatToAttrList("repeatS", tvalb);

            // repeatT="true" SFBool
            if (material.Get(AI_MATKEY_MAPPINGMODE_V_DIFFUSE(0), tvalb) == AI_SUCCESS)
                RepeatToAttrList("repeatT", tvalb);
        ...
        //
        {
            auto Vec2ToAttrList = [&](const string &pAttrName, const aiVector2D &pAttrValue,
                                      const aiVector2D &pAttrDefaultValue) {
                string tstr;
    ...
    bool X3DExporter::CheckAndExport_Light(const aiNode &pNode, const size_t pTabLevel) {
        ...
        auto Vec3ToAttrList = [&](const string &pAttrName, const aiVector3D &pAttrValue,
        ...
        AttrHelper_FloatToAttrList(attr_list, "ambientIntensity",
                                   aiVector3D(light.mColorAmbient.r, light.mColorAmbient.g,
                                              light.mColorAmbient.b).Length(), 0);
        ...
        switch (light.mType) {
            ...
            case aiLightSource_POINT: {
                aiVector3D attenuation(light.mAttenuationConstant, light.mAttenuationLinear,
    ...
    X3DExporter::X3DExporter(const char *pFileName, IOSystem *pIOSystem, const aiScene *pScene,
