...
static CVReturn
MyDisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow, const CVTimeStamp *inOutputTime,
    ...
    [openPanel beginSheetModalForWindow:[_view window] completionHandler:^(NSInteger result) {
        if (result == NSOKButton) {
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:[openPanel filename]]) {
                ...
                _scene = (aiScene *) aiImportFileExWithProperties(
                        [[openPanel filename] cStringUsingEncoding:[NSString defaultCStringEncoding]],
...
- (void)createGLResourcesInContext:(CGLContextObj)cgl_ctx {
    ...
    for (unsigned int i = 0; i < _scene->mNumMeshes; ++i) {
        ...
        if (AI_SUCCESS == mtl->GetTexture(aiTextureType_DIFFUSE, texIndex, &texPath)) {
            ...
            //bind texture
            NSNumber * textureNumber = (NSNumber * )
            [textureDictionary valueForKey:textureKey];
...
- (void)loadTexturesInContext:(CGLContextObj)cgl_ctx withModelPath:(NSString *)modelPath {
    ...
    textureIds = (GLuint *) malloc(
            sizeof(GLuint) * [textureDictionary count]); //new GLuint[ [textureDictionary count] ];
