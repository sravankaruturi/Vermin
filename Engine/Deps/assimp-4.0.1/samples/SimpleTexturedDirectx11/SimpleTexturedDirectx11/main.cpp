...
HRESULT
CompileShaderFromFile(LPCWSTR pFileName, const D3D_SHADER_MACRO *pDefines, LPCSTR pEntryPoint, LPCSTR pShaderModel,
...
QueryInterface(__uuidof(ID3D11DeviceContext1),
...
dev->
CreateRenderTargetView(pBackBuffer, NULL, &backbuffer);
pBackBuffer->
...
dev->
CreateRasterizerState(&rasterDesc, &rasterState
);
devcon->
RSSetState(rasterState);
...
CompileShaderFromFile(LPCWSTR pFileName, const D3D_SHADER_MACRO *pDefines, LPCSTR pEntryPoint, LPCSTR pShaderModel,
    ...
    HRESULT result = D3DCompileFromFile(pFileName, pDefines, D3D_COMPILE_STANDARD_FILE_INCLUDE, pEntryPoint,
                                        pShaderModel, compileFlags, 0, ppBytecodeBlob, &pErrorBlob);
    if (FAILED(result)) {
        if (pErrorBlob != NULL)
