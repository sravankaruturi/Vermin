...
public:
    ...
    virtual void SetUp() {
        ...
        pTest = im->ReadFile(ASSIMP_TEST_MODELS_DIR
        "/X/test.x", 0);
    }
...
EXPECT_TRUE(im
->
ReadFileFromMemory(blob
->data, blob->size, 0, "dae"));
}
...
EXPECT_TRUE(desc
->
description &&strlen(desc->description)
);
EXPECT_TRUE(desc
->
fileExtension &&strlen(desc->fileExtension)
);
EXPECT_TRUE(desc
->
id &&strlen(desc->id)
...
{
EXPECT_TRUE (aiGetExportFormatCount()

> 0);
for (
size_t i = 0;
i<aiGetExportFormatCount();
++i) {
