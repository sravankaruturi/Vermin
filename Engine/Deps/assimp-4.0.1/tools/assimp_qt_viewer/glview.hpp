...
private:
    ...
    struct SHelper_Mesh {
        ...
        explicit SHelper_Mesh(const size_t pQuantity_Point, const size_t pQuantity_Line,
                              const size_t pQuantity_Triangle, const SBBox &pBBox = {{0, 0, 0},
                                                                                     {0, 0, 0}});
...
private:
    ...
    void AssignIfLesser(TArg *pBaseValue, const TArg pTestValue) {
        if (pTestValue < *pBaseValue)
            *pBaseValue = pTestValue;
    }

    template<typename TArg>
    void AssignIfGreater(TArg *pBaseValue, const TArg pTestValue) {
        if (pTestValue > *pBaseValue)
            *pBaseValue = pTestValue;
    }
    ...
    void BBox_GetForNode(const aiNode &pNode, const aiMatrix4x4 &pParent_TransformationMatrix, SBBox &pNodeBBox,
                         bool &pFirstAssign);
...
public:
    ...
    Paint_Finished(const size_t pPaintTime_ms, const GLfloat pDistance);
