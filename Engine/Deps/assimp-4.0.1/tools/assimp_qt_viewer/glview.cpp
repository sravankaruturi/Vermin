...
CGLView::SHelper_Mesh::SHelper_Mesh(const size_t pQuantity_Point, const size_t pQuantity_Line,
                                    const size_t pQuantity_Triangle, const SBBox &pBBox)
        : Quantity_Point(pQuantity_Point), Quantity_Line(pQuantity_Line), Quantity_Triangle(pQuantity_Triangle),
          BBox(pBBox) {
...
void CGLView::Material_Apply(const aiMaterial *pMaterial) {
    ...
    max = 1;
    if ((AI_SUCCESS == aiGetMaterialIntegerArray(pMaterial, AI_MATKEY_TWOSIDED, &two_sided, &max)) &&
        two_sided)///TODO: cache
...
void CGLView::ImportTextures(const QString &pScenePath) {
    auto LoadTexture = [&](const QString &pFileName) -> bool ///TODO: IME texture mode, operation.
    {
        ...
        if (!pFileName.startsWith(AI_EMBEDDED_TEXNAME_PREFIX)) {
            ...
            //Cleanup
            ilDeleteImages(1,
                           &id_image);// Because we have already copied image data into texture data we can release memory used by image.
        } else {
            ...
            if (als.mHeight == 0)// Compressed texture.
            {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, Pixel_Description[idx_description].Image_InternalFormat, als.mWidth,
                             als.mHeight, 0,
    ...
    for (size_t idx_material = 0; idx_material < mScene->mNumMaterials; idx_material++) {
        ...
        do {
            if (mScene->mMaterials[idx_material]->GetTexture(aiTextureType_DIFFUSE, idx_texture, &path) !=
                AI_SUCCESS)
                break;
...
void CGLView::BBox_GetForNode(const aiNode &pNode, const aiMatrix4x4 &pParent_TransformationMatrix, SBBox &pNodeBBox,
                              bool &pFirstAssign) {
...
void CGLView::Draw_BBox(const SBBox &pBBox) {
    ...
    glVertex3fv(&vertex[0][0]), glVertex3fv(&vertex[1][0]), glVertex3fv(&vertex[2][0]), glVertex3fv(
            &vertex[3][0]), glVertex3fv(&vertex[0][0]);// "Minimum" side.
    glVertex3fv(&vertex[4][0]), glVertex3fv(&vertex[5][0]), glVertex3fv(&vertex[6][0]), glVertex3fv(
            &vertex[7][0]), glVertex3fv(&vertex[4][0]);// Edge and "maximum" side.
    glEnd();
    glBegin(GL_LINES);
...
void CGLView::paintGL() {
    ...
    glTranslatef(-mHelper_Camera.Translation_ToScene.x, -mHelper_Camera.Translation_ToScene.y,
                 -mHelper_Camera.Translation_ToScene.z);
    ...
    emit Paint_Finished((size_t) time_paintbegin.msecsTo(QTime::currentTime()),
                        mHelper_Camera.Translation_ToScene.Length());
}
...
CGLView::CGLView(QWidget *pParent)
    ...
    static_assert(sizeof(GLfloat) == sizeof(ai_real),
                  "ai_real in Assimp must be equal to GLfloat/float.");///TODO: may be templates can be used.
...
void CGLView::SetScene(const aiScene *pScene, const QString &pScenePath) {
    ...
    if (!mScene->HasLights()) {
        ...
        for (size_t idx_light = 0; idx_light < mScene->mNumLights; idx_light++) {
            ...
            auto col3_to_col4 = [](const aiColor3D &pCol3) -> aiColor4D {
                return aiColor4D(pCol3.r, pCol3.g, pCol3.b, 1.0);
            };
    ...
    if (!mScene->HasCameras()) {
        ...
        GLfloat val_x =
                ((mScene_BBox.Maximum.x - mScene_BBox.Minimum.x) / 2) / (mCamera_Viewport_AspectRatio * tg_angle);
        GLfloat val_y = ((mScene_BBox.Maximum.y - mScene_BBox.Minimum.y) / 2) / tg_angle;
        GLfloat val_step = val_x;

        AssignIfGreater(val_step, val_y);
        mHelper_CameraDefault.Translation_ToScene.Set(mScene_Center.x, mScene_Center.y,
                                                      val_step + mScene_BBox.Maximum.z);
...
void CGLView::Lighting_EditSource(const size_t pLightNumber, const SLightParameters &pLightParameters) {
    ...
    switch (pLightParameters.Type) {
        case aiLightSource_DIRECTIONAL:
            ...
            farr[2] = pLightParameters.For.Directional.Direction.z;
            farr[3] = 0;
        ...
        case aiLightSource_POINT:
            ...
            farr[2] = pLightParameters.For.Point.Position.z;
            farr[3] = 1;
            glLightfv(light_num, GL_POSITION, farr);
            // Attenuation
        ...
        case aiLightSource_SPOT:
            // Position
            farr[0] = pLightParameters.For.Spot.Position.x, farr[2] = pLightParameters.For.Spot.Position.y, farr[2] = pLightParameters.For.Spot.Position.z;
            farr[3] = 1;
            glLightfv(light_num, GL_POSITION, farr);
            // Attenuation
            ...
            // Spot specific
            farr[0] = pLightParameters.For.Spot.Direction.x, farr[2] = pLightParameters.For.Spot.Direction.y, farr[2] = pLightParameters.For.Spot.Direction.z;
            farr[3] = 0;
...
void CGLView::Camera_Set(const size_t pCameraNumber) {
    ...
    if (mCamera_DefaultAdded ||
        (pCameraNumber >= mScene->mNumCameras))// If default camera used then 'pCameraNumber' doesn't matter.
    {
    ...
    gluLookAt(hcam.Position.x, hcam.Position.y, hcam.Position.z, hcam.Target.x, hcam.Target.y, hcam.Target.z, up.x,
              up.y, up.z);
}
