...
public:
    template<class T>
    SmartLocalRef(JNIEnv *env, T &object)
            : mJniEnv(env), mJavaObj((jobject & )
...
static bool createInstance(JNIEnv *env, const char *className, const char *signature,/* const*/ jvalue *params,
                           jobject &newInstance) {
    jclass clazz = env->FindClass(className);
    SmartLocalRef clazzRef(env, clazz);
...
static bool
getStaticField(JNIEnv *env, const char *className, const char *fieldName, const char *signature, jobject &field) {
    jclass clazz = env->FindClass(className);
    SmartLocalRef clazzRef(env, clazz);
...
static bool copyBufferArray(JNIEnv *env, jobject jMesh, const char *jBufferName, int index, void *cData, size_t size) {
    ...
    jobject
    jBuffer = env->GetObjectArrayElement((jobjectArray) jBufferArray, index);
    SmartLocalRef bufferRef(env, jBuffer);
...
static bool loadMeshes(JNIEnv *env, const aiScene *cScene, jobject &jScene) {
    for (unsigned int meshNr = 0; meshNr < cScene->mNumMeshes; meshNr++) {
        ...
        jstring
        nameString = env->NewStringUTF(cMesh->mName.C_Str());
        ...
        allocateBuffersParams[3].i = (jint)
        faceBufferSize;
        ...
        for (int c = 0; c < AI_MAX_NUMBER_OF_COLOR_SETS; c++) {
            if (cMesh->mColors[c] != NULL) {
                ...
                if (!copyBufferArray(env, jMesh, "m_colorsets", c, cMesh->mColors[c],
                                     cMesh->mNumVertices * 4 * sizeof(float))) {
        ...
        for (int c = 0; c < AI_MAX_NUMBER_OF_TEXTURECOORDS; c++) {
            if (cMesh->mTextureCoords[c] != NULL) {
                ...
                for (unsigned int v = 0; v < cMesh->mNumVertices; v++) {
                    memcpy(coordBuffer + coordBufferOffset, &cMesh->mTextureCoords[c][v],
                           cMesh->mNumUVComponents[c] * sizeof(float));
        ...
        for (unsigned int b = 0; b < cMesh->mNumBones; b++) {
            ...
            /* set bone data */
            jstring
            boneNameString = env->NewStringUTF(cBone->mName.C_Str());
            ...
            for (unsigned int w = 0; w < cBone->mNumWeights; w++) {
                ...
                if (!callStaticObject(env, "jassimp/Jassimp", "wrapMatrix", "([F)Ljava/lang/Object;", wrapParams,
                                      jMatrix)) {
                    return false;
                }
...
static bool loadSceneNode(JNIEnv *env, const aiNode *cNode, jobject parent, jobject *loadedNode = NULL) {
    ...
    jint * temp = (jint * )
    malloc(sizeof(jint) * cNode->mNumMeshes);

    for (unsigned int i = 0; i < cNode->mNumMeshes; i++) {
        temp[i] = cNode->mMeshes[i];
    }
    env->SetIntArrayRegion(jMeshrefArr, 0, cNode->mNumMeshes, (jint * )
    temp);
    ...
    /* convert name */
    jstring
    jNodeName = env->NewStringUTF(cNode->mName.C_Str());
...
static bool loadMaterials(JNIEnv *env, const aiScene *cScene, jobject &jScene) {
    for (unsigned int m = 0; m < cScene->mNumMaterials; m++) {
        ...
        for (unsigned int p = 0; p < cMaterial->mNumProperties; p++) {
            ...
            jstring
            keyString = env->NewStringUTF(cProperty->mKey.C_Str());
            ...
            if (NULL != strstr(cProperty->mKey.C_Str(), "clr") &&
                ...
                if (!callStaticObject(env, "jassimp/Jassimp", "wrapColor3", "(FFF)Ljava/lang/Object;", wrapColorParams,
                                      jData)) {
                    return false;
                }
                ...
                if (!callStaticObject(env, "jassimp/Jassimp", "wrapColor4", "(FFFF)Ljava/lang/Object;", wrapColorParams,
                                      jData)) {
                    return false;
                }
                ...
                /* skip length prefix */
                jobject
                jData = env->NewStringUTF(cProperty->mData + 4);
                SmartLocalRef refData(env, jData);
                ...
                if (!createInstance(env, "jassimp/AiMaterial$Property", "(Ljava/lang/String;IIII)V", constructorParams,
                                    jProperty)) {
                    return false;
                }
...
static bool loadAnimations(JNIEnv *env, const aiScene *cScene, jobject &jScene) {
    ...
    for (unsigned int a = 0; a < cScene->mNumAnimations; a++) {
        ...
        jstring
        nameString = env->NewStringUTF(cAnimation->mName.C_Str());
        SmartLocalRef refNameString(env, nameString);
        ...
        for (unsigned int c = 0; c < cAnimation->mNumChannels; c++) {
            ...
            jstring
            animationName = env->NewStringUTF(cNodeAnim->mNodeName.C_Str());
            ...
            if (!createInstance(env, "jassimp/AiNodeAnim", "(Ljava/lang/String;IIIII)V", newNodeAnimParams,
                                jNodeAnim)) {
                return false;
            }
...
static bool loadLights(JNIEnv *env, const aiScene *cScene, jobject &jScene) {
    ...
    for (unsigned int l = 0; l < cScene->mNumLights; l++) {
        ...
        if (!callStaticObject(env, "jassimp/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapVec3Params,
                              jPosition)) {
            return false;
        }
        ...
        jvalue params[12];
        jstring
        lightName = env->NewStringUTF(cLight->mName.C_Str());
        ...
        if (!createInstance(env, "jassimp/AiLight",
...
static bool loadCameras(JNIEnv *env, const aiScene *cScene, jobject &jScene) {
    ...
    for (unsigned int c = 0; c < cScene->mNumCameras; c++) {
        ...
        jPosition;
        SmartLocalRef refPosition(env, jPosition);
        if (!callStaticObject(env, "jassimp/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapPositionParams,
                              jPosition)) {
            return false;
        }
        ...
        jvalue params[8];
        jstring
        cameraName = env->NewStringUTF(cCamera->mName.C_Str());
        ...
        if (!createInstance(env, "jassimp/AiCamera",
...
JNICALL Java_jassimp_Jassimp_getVKeysize
...
JNICALL Java_jassimp_Jassimp_getQKeysize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getV3Dsize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getfloatsize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getintsize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getuintsize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getdoublesize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getlongsize
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_getErrorString
        (JNIEnv *env, jclass jClazz) {
...
JNICALL Java_jassimp_Jassimp_aiImportFile
