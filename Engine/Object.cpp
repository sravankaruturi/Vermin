...
namespace vermin {
    ...
    void CalcInterpolatedScaling(aiVector3D &_out, float _animationTime, const aiNodeAnim *_nodeAnim) {
        ...
        auto delta_time = (float) (_nodeAnim->mScalingKeys[nex_sca_index].mTime -
                                   _nodeAnim->mScalingKeys[scaling_index].mTime);
    ...
    Object::Object(const std::string &_objectPath) {
        ...
        objectName = _objectPath.substr(_objectPath.find_last_of('/') + 1,
                                        _objectPath.find_last_of('.') - _objectPath.find_last_of('/') - 1);
        ...
        assimpScene = assimpImporter.ReadFile(_objectPath,
                                              aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        if (!assimpScene || assimpScene->mFlags & AI_SCENE_FLAGS_INCOMPLETE ||
            !assimpScene->mRootNode) // if is Not Zero
        {
    ...
    void Object::BoneTransform(float _totalTime, std::vector<glm::mat4> &_matrices) {
        ...
        const auto ticks_per_second = assimpScene->mAnimations[selected_animation_index]->mTicksPerSecond != 0
                                      ? assimpScene->mAnimations[0]->mTicksPerSecond : 25.0f;
        ...
        ProcessNodeHierarchyAnimation(animation_time, assimpScene->mRootNode, selected_animation_index,
                                      root_node_matrix);
    ...
    void Object::ProcessAndAddMesh(aiMesh *_mesh, const aiScene *_scene) {
        ...
        if (_mesh->HasBones()) {
            ...
            std::shared_ptr<Mesh> return_renderable = std::make_shared<Mesh>(&vertices[0], sizeof(AnimatedVertexData),
                                                                             vertices.size(), indices);
            ...
            std::shared_ptr<Mesh> return_renderable = std::make_shared<Mesh>(&vertices[0], sizeof(VertexData),
                                                                             vertices.size(), indices);
            return_renderable->SetTextureNames(textures);
    ...
    void Object::ProcessNodeHierarchyAnimation(float _animationTime, const aiNode *_node, const int _animationIndex,
        ...
        for (auto i = 0; i < _node->mNumChildren; i++) {
            ProcessNodeHierarchyAnimation(_animationTime, _node->mChildren[i], selectedAnimationIndex,
