...
namespace vermin {
	...
	void TestScene::InitEntities()
	{
		...
		/* Initialize Cameras */
		cameras.insert(std::make_pair("First", std::make_shared<Camera>("First", glm::vec3(0, 5, 10), glm::vec3(0.5, -0.5, -0.5), glm::vec3(0, 1, 0))));
		cameras.insert(std::make_pair("Second", std::make_shared<Camera>("Second", glm::vec3(10, 5, 10), glm::vec3(-0.5, -0.5, -0.5), glm::vec3(0, 1, 0))));
	...
	void TestScene::LoadScene(const char * _fileName)
	{
		...
		if (in.good() && !in.eof()) {
			...
			for (auto i = 0; i < number_of_cameras; i++) {
				...
				cameras.insert(std::make_pair(camera_key, cam));
	...
	void TestScene::HandleInputs()
	{
...

		{
			...
			{
				this->GetActiveCamera()->ProcessKeyboard(vermin::Camera::forward, deltaTime);
			}

			if (window->IsKeyPressedOrHeld(GLFW_KEY_S))
			{
				this->GetActiveCamera()->ProcessKeyboard(vermin::Camera::back, deltaTime);
			}

			if (window->IsKeyPressedOrHeld(GLFW_KEY_A))
			{
				this->GetActiveCamera()->ProcessKeyboard(vermin::Camera::leftside, deltaTime);
			}

			if (window->IsKeyPressedOrHeld(GLFW_KEY_D))
			{
				this->GetActiveCamera()->ProcessKeyboard(vermin::Camera::rightside, deltaTime);
			}
